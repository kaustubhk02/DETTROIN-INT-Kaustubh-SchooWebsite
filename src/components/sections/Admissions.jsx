...
export default function Admissions({ onOpenInquiry }) {
  return (
    <section id="admissions" className="py-16 sm:py-24 bg-slate-900 text-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {ADMISSION_STEPS.map((item, idx) => (
            <div
              key={idx}
              className="bg-slate-800/90 rounded-2xl p-6 border border-slate-700 relative flex flex-col justify-between hover:border-amber-400/50 transition-colors"
            >
              <div>
                <div className="text-3xl font-black text-amber-400 mb-3 font-mono">
                  {item.step}
                </div>
        ...
        <div className="mt-12 bg-gradient-to-r from-blue-700 to-indigo-800 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl border border-blue-500/30">
          ...
          <button
            ...
            className="px-6 py-3.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-bold text-sm rounded-xl shadow-lg transition-colors flex items-center gap-2 shrink-0 cursor-pointer"
          >
            <Send size={16} />
